...
data = load_data('C:\Users\Neil\SkyDrive\University\HonoursProject\annotated_images', 1:1);
...
for m = 1:length(data.image_filename)
    ...
    mask{m} = imread(data.gt_filename{m});
    mask{m} = rgb2gray(mask{m});
    mask{m}(mask{m} > 0 & mask{m} < 255) = 1;  % all gray areas to 1
    mask{m}(mask{m} == 255) = 2;  % all white areas to 2
end
